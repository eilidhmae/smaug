...
**Open items blocking Wave-A/B/C plan execution:**
- [ ] External adversary pass on Wave C new plans: stances-olc, auction, clan-officer. Use `manager` subagent type — note: all 2026-04-18 manager audits reported `Agent` tool unavailable in the harness despite spec listing it; structured self-review substituted. Separate tooling investigation tracked below.
...
- [ ] Auction plan Q4 (human input): carry-weight check on obj-to-char vs obj-to-room in sold/unsold branches. Default: skip the check; always `ObjToChar`. Plan documents C-verbatim alternative.
...
- [ ] Auction plan Q4 sign-off — audit Error 3 correction invalidated the "unreachable branch" rationale for the carry-weight check. Human review needed on whether "always ObjToChar" fallback is acceptable given the unsold branch IS genuinely reachable.
...
- [ ] **Full `do_auction` state machine** — `plan-phase6-auction.md` drafted 2026-04-18 via `phase6-auction` lineage. 10 groups, 23 criteria, 8 open questions. Non-GSC variant; completes Tier-9 `BroadcastAuction` stub. Ports `act_obj.c:3775-4240` + `update.c:3183-3326`. Two-layer disconnect defense. 17 C bugs/anomalies catalogued. External adversary queued.
