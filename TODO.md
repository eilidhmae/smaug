...
- [x] Wave 2 (commit TBD) — G2/G3 real menu renderers (replacing Wave-1 stubs: NPC main-menu + PC main-menu + 16 G3 submenu renderers as display-only scaffolds), G5 NPC main-menu dispatch (34-digit table), G6 PC main-menu dispatch (30-digit table with LEVEL_GREATER on LEVEL/CLASS/RACE + LEVEL_GOD on `Z` Clan + LEVEL_SUB_IMPLEM on `=` Council + verbatim "NPC Only!!"/"NPCs Only!!" reject strings), MEDIT_CONFIRM_SAVESTRING Y/N-both-cleanup stub, `act.MeditDispMenuFunc = game.MeditDispMenu` boot wire + pin-test. ~140 new subtests; 9 mutation gates via `Edit` round-trips. Adversary returned CONCERNS → A12 gap (Z/= digits missing from `pcDigitTable`) closed via follow-up pass that also added `TestMeditDispatchPcMain_TrustGateOnClanCouncil`; production audit confirmed no C-parity bug (test-only coverage gap).
- [ ] Wave 2 follow-ups (captured for Waves 3-5):
  - Full `MEDIT_CONFIRM_SAVESTRING` save pathway (plan G11) — Wave 2 Y/N-both-cleanup stub; Y must eventually invoke `save_char_obj` before cleanup. Pin-test `TestMeditConfirmSavestring_YInvokesSave` deferred to G11.
  - Plan §597-607 vs C `omedit.c:1247-1253` divergence: plan puts LEVEL_GREATER gate on PC digits 4 (CLASS) / 5 (RACE) at dispatch time; C applies it in the arm body. Wave 2 followed the plan; G12 must reconcile when it ports the arm bodies — either drop the dispatch-time gate or document the divergence in the plan.
  - Wave 2 flag-submenu renderers (Attacks / Defenses / NpcFlags / PcFlags / AffFlags / PcdataFlags / Parts) render masks as hex / `BitVector.String()` because flag-name tables (`attack_flags` / `defense_flags` / `act_flags` / `plr_flags` / `aff_flags` / `pc_flags` / `part_flags`) don't exist yet. G9 must port all seven tables as part of the shared `olcBitmaskEdit` helper; renderers back-wire to the named tables then.
  - `util.SmashTilde` not applied to Wave-2 submenu-prompt strings — Wave 2 does not yet accept submenu input. Simple-field arms (G7) must call `SmashTilde` on all string-field inputs before assignment (plan §G7 already calls this out; flagging so it doesn't slip).
  - C bug preserved: C PC-menu default arm (`omedit.c:1382-1384`) incorrectly calls `medit_disp_npc_menu` from the PC dispatcher; Go calls `meditDispPcMenu` (intentional divergence, documented inline at `medit_parse.go:382-389`). Consider filing an upstream issue.
  - **medit G9 prerequisite** — `internal/game/medit_menu.go:455` `meditDispPcFlagsMenu` renders `victim.Act.String()` (uses `act_flags` naming) while the submenu label says "PLR_*". Data source is correct; display vocabulary mismatches. Must be resolved when G9 bitmask edit helper lands (Wave 3+) — same wave that introduces the `plr_flags` name table.
  - **medit G11 polish** — `internal/game/medit_parse.go:414` `MEDIT_CONFIRM_SAVESTRING` default-reprompt text is `"Please answer Y or N: "` while C `omedit.c:1053-1054` says `"Invalid choice!\n\r"` + `"Do you wish to save to disk? : "`. Harmonize when the full G11 save-confirm body lands.
