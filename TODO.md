...
### Phase 6 OLC oedit (landing in waves)
...
- [x] Wave 4 — G10 `DoOedit` no-arg menu-entry path extension (replaces `oeditShow` call at `internal/act/olc_interactive.go:72` — opens CON_OEDIT when no subcommand given; old summary rebound as explicit `oedit <vnum> show`). 7 unit tests: `TestDoOedit_NoArgPrintsUsage`, `TestDoOedit_VnumOnlyEntersMenu`, `TestDoOedit_UnknownVnumRefuses`, `TestDoOedit_NonImmortalRefused`, `TestDoOedit_NoDescriptor`, `TestDoOedit_FlatPathStillWorks`, `TestDoOedit_ShowSubcommand`. 2 mutation gates verified via `Edit` round-trips.
- [x] Wave 4 — G12 testclient E2E scenarios in new `internal/testclient/oedit_test.go`. 6 scenarios: `TestTestclient_OeditMenuEntryAndQuit`, `TestTestclient_OeditMenuSetName`, `TestTestclient_OeditInvalidChoiceRedisplays`, `TestTestclient_OeditTypeSubmenu`, `TestTestclient_OeditLongdescRoundtrip` (pins `Connected=CON_OEDIT` post-`/s`), `TestTestclient_OeditAreaSaveGuardsPathTraversal`. 3 mutation gates verified (loop.go CON_OEDIT→CON_MEDIT, oedit_parse.go longdesc-closure CON_OEDIT→CON_PLAYING, DoSaveArea path-containment guard dropped).
- [x] Wave 4 — G13 CHANGELOG entry + CLAUDE.md phase table "LANDED 2026-04-19" update with 4-wave commit list + 100% A1-A27 coverage + C-bug/divergence summary. Plan row and "Still unauthored" paragraph both updated.
...
- [x] Wave 2 (commit `c9bf798`) — G2/G3 real menu renderers (NPC main-menu + PC main-menu + 16 G3 submenu renderers as display-only scaffolds), G5 NPC main-menu dispatch (34-digit table), G6 PC main-menu dispatch (30-digit table with LEVEL_GREATER on LEVEL/CLASS/RACE + LEVEL_GOD on `Z` Clan + LEVEL_SUB_IMPLEM on `=` Council + verbatim "NPC Only!!"/"NPCs Only!!" reject strings), MEDIT_CONFIRM_SAVESTRING Y/N-both-cleanup stub, `act.MeditDispMenuFunc = game.MeditDispMenu` boot wire + pin-test. ~140 new subtests; 9 mutation gates. Adversary CONCERNS → A12 gap (Z/= digits) closed via follow-up + `TestMeditDispatchPcMain_TrustGateOnClanCouncil`.
- [x] Wave 3 (commit TBD) — G7 25 simple-field arms + G8 7 stat editors (NPC `uRange(1,25)` / PC `uRange(3,18)` + ACT_PROTOTYPE prototype mirror) + G9 shared `olcBitmaskEdit` helper (int + BitVector variants, exit semantics done/quit/empty/0) + 8 flag-arm wires (ACT/PLR/AFF/PCFLAG/PART/RIS×3) + 5 new flag-name tables in `util/flags.go` (`ActflagNames`/`PlrflagNames`/`AffflagNames`/`PcflagNames`/`PartflagNames` — bit-for-bit C `build.c:216-265` parity including reserved `r1..r10` slots) + Wave-2 flag-submenu renderer back-wires (no more hex/`BitVector.String()`) + oedit `OEDIT_AFFECT_MODIFIER` back-wire to consume `olcBitmaskEdit`. ~58 new tests, 16 mutation gates. Adversary returned CONCERNS — fixed in same commit: HIGH #1 stat-prototype mirror omitted (false-premise comment claimed `MobIndexData.PermStr..PermLck` missing — they exist at `mob_index.go:67-73`); HIGH #2 ACT_IS_NPC keyword-bypass via `protectedToggleRejected` numeric-only guard (closed via snapshot/restore + verbatim C string `"It isn't possible to change that flag.\n\r"`); MEDIUM #3 ACT_PROTOTYPE keyword bypass (same pattern + verbatim C string `"You don't have permission to change the prototype flag.\n\r"`); LOW #4 spurious mirror on `done` (moved inside `if !done` branch in 4 bitmask arms); LOW #5 flag tables didn't match C bit-for-bit (added trailing reserved slots); LOW #6 MEDIT_SEX rejected vs C-clamp (switched to `URANGE(0,x,2)` per C parity).
- [ ] Wave 3 follow-ups (queued for Waves 4-5):
  - **medit MEDIT_ATTACK / MEDIT_DEFENSE keyword lookup** — Wave 3 wires accept numeric indices only. Add `util.AttackflagNames` and `util.DefenseflagNames` tables (per C `build.c:334-351`) and extend the arms. Pin-test: keyword "bite" toggles attack bit 0.
  - **medit MEDIT_NAME PC branch — `do_pcrename`** — when a PC's name changes, C `omedit.c:1390-1395` invokes `do_pcrename(ch, "<old> <new>")` which also renames the player's pfile on disk. Go port currently does in-memory `victim.Name = arg` only; pfile is stale until next save. Port `do_pcrename` (or fold the rename into `act.SaveFunc`) and switch the trust-gated branch to call it.
  - **medit MEDIT_CLAN/DEITY/COUNCIL pointer resolution** — Wave 3 stores name string verbatim into `PCData.ClanName`/`DeityName`/`CouncilName` but does NOT resolve the pointer (`PCData.Clan`/`Deity`/`Council`). C `omedit.c:2165-2262` looks up via `get_deity` / `get_clan` / `get_council` and validates; failed lookups bug-log and skip. Add `world.GetDeity`/`GetClan`/`GetCouncil` (grep — may already exist) and wire them. Until then the strings are display-only.
  - Full `MEDIT_CONFIRM_SAVESTRING` Y-path that invokes `act.SaveFunc(victim)` (Wave 2 stub currently routes Y → cleanupOlc without saving). Plan §G11.
  - Plan §597-607 vs C `omedit.c:1247-1253` divergence: plan puts LEVEL_GREATER gate on PC digits 4 (CLASS) / 5 (RACE) at dispatch time; C applies it in the arm body. Wave 2 followed the plan; G12 must reconcile when it ports the arm bodies.
  - C bug preserved: C PC-menu default arm (`omedit.c:1382-1384`) incorrectly calls `medit_disp_npc_menu` from the PC dispatcher; Go calls `meditDispPcMenu` (intentional divergence, documented inline at `medit_parse.go:382-389`).
  - **medit G11 polish** — `MEDIT_CONFIRM_SAVESTRING` default-reprompt text is `"Please answer Y or N: "` while C `omedit.c:1053-1054` says `"Invalid choice!\n\r"` + `"Do you wish to save to disk? : "`. Harmonize when full G11 save-confirm body lands.
  - `worldPcLookup` seam — plan §540 claims Wave 1 added it, but only `worldMobLookup` exists. Not needed until G14 `DoMedit` supports `medit <pc-name>` (Wave 5).
- [x] Wave 4 (commit TBD) — G10 full affect editor (4 new MEDIT_AFFECT_* modes at iota+364-367; Add/Remove/Modifier flow; bitmask APPLY_AFFECT/RESISTANT/IMMUNE/SUSCEPTIBLE via Wave-3 `olcBitmaskEdit`) + G11 save-throw editor (5-way digit dispatch, 5 SAV arms via `meditArmSav` helper writing `SavingPoisonDeath`/`Wand`/`ParaPetri`/`Breath`/`SpellStaff` with C `URANGE(-30,30)` clamp; ACT_PROTOTYPE mirror) + G11 `MEDIT_CONFIRM_SAVESTRING` Y-path full port (Y → `act.SaveFunc(victim)` → cleanup; N → cleanup; verbatim C reprompt strings replacing Wave-2 stub) + G12 CLASS/RACE editors (numeric + name lookup via `worldRef.Classes[i].WhoName` / `Races[i].Name`; defense-in-depth trust gate; **NPC + PC both accepted per C `:1866-1890`** — plan said PC-only, Go followed C) + G13 MEDIT_PASSWORD security-critical via bcrypt (min-length 5; pre-hash tilde reject; `LEVEL_SUB_IMPLEM` trust gate per C `:1602` — plan §708 hinted LEVEL_GREATER, Go followed C; 6 security pin-tests #1-6 from plan §432-440). 30 new tests; 11 mutation gates verified (G10×3, G11×3, G12×2, G13×3). Adversary CONCERNS — 1 MEDIUM (test misleadingly runs through fallback path always — `TestMeditAffect_AddBitmaskPath_AffectFlag` hardcoded "27" guess, recovery path always taken; functionally correct, structurally misleading) + 2 LOW (`util.SmashTilde` no-op call after tilde-reject; `intInput` test helper re-implements `strconv.Itoa`). G13 security audit PASSED (no plaintext leakage, no hash bypass, no trust-gate bypass, olcLog never includes plaintext or hash).
- [ ] Wave 4 follow-ups (queued for Wave 5 cleanup or later):
  - **Test cleanup (Wave-4 adversary MEDIUM)**: `medit_wave4_test.go:524-535` `TestMeditAffect_AddBitmaskPath_AffectFlag` always runs through the recovery path because the hardcoded "27" first-attempt is wrong (APPLY_AFFECT=26). Replace with `intInput(types.APPLY_AFFECT)` + remove the recovery block. Functionally green today; structurally misleading.
  - **Test cleanup (Wave-4 adversary LOW)**: `medit_wave4_arms.go:395-396` `util.SmashTilde(arg)` is a no-op after the prior tilde-rejection. Remove or replace the comment with explicit "intentional no-op for parity with other arms" wording.
  - **Test cleanup (Wave-4 adversary LOW)**: `medit_wave4_test.go:568-586` `intInput` re-implements `strconv.Itoa`. Replace with stdlib import.
  - **`util.ApplyflagNames` table** for name-lookup at MEDIT_AFFECT_LOCATION — Wave 4 G10 takes numeric APPLY_* only. Cross-cuts to oedit `OEDIT_AFFECT_LOCATION` which has the same numeric-only constraint (a Wave-3 follow-up).
  - **Mob-prototype `fold_area` writer**: C `omedit.c:1014-1043` runs `fold_area` on the prototype's owning area when the builder confirms save on an NPC mob. Wave 4 reuses `act.SaveFunc` (PC pfile writer; no-op for NPCs per `loop.go:813` IsNPC guard). Land a true mob-prototype area-writer (likely via `oedit.DoSaveArea` reuse) and dispatch the Y-path NPC branch to it.
  - **`MobIndexData.Affects` field**: C mobs persist permanent affects on the prototype (`pIndexData->first_affect`); the Go `MobIndexData` struct has no `Affects` field. Wave 4 G10 documents this inline (`medit_affect.go` file-header note) — affect-list edits are instance-only and don't propagate to subsequent NPC instantiations. Add `Affects []*AffectData` to `MobIndexData`, wire the prototype mirror in `meditAffectCommit` / `meditAffectRemoveIndex`, extend the file loader.
  - **`MEDIT_AFFECT_MENU` main-menu digit**: Wave-4 affect-list editor is reachable only by direct mode-set today (tests + future programmatic dispatch). Existing AFF_FLAGS digit (NPC-Z / PC-X) does bitmask-only toggles per C. Future digit (or sub-prompt off AFF_FLAGS menu) should expose the new affect-list editor — Wave 5 / G14 candidate.
  - **`util.ClassLookup` / `util.RaceLookup`** promotion: G12 inlines the lookup at the medit call site. Promoting to `internal/util/` would let oedit / mset / future class-aware editors reuse the same name-resolver.
  - **`MEDIT_AFFECT_LOCATION` quit semantics** (LOW): no `q`/`quit` escape — builder must send `0` to cancel. Matches oedit but not documented in the prompt text.
- [ ] Wave 5: G14 DoMedit menu-entry extension (mirrors DoOedit pattern from oedit Wave 4). Note: oedit `OEDIT_AFFECT_MODIFIER` back-wire to `olcBitmaskEdit` already landed in Wave 3 — that part of the plan-stated G14 deliverable is closed.
