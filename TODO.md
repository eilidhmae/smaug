...
1. **`plan-do-channels.md`** (P1) — Fix `Deaf`-bitvector persistence BUG (silent data loss on every logout) + ship `DoChannels` toggle command with full 30-entry channel table. **G1 (persistence) landed 2026-04-17; G2-G5 (DoChannels command + tests + registration + docs) is the remaining work.**
...
- [ ] `DoChannels` toggle command (re-enable a deaf channel without reconnecting). Required pair for the deaf-not-cleared behavior in `DoImmtalk` per C semantics — today a player who runs `channels -immtalk` has no way to reverse it in the Go port.
