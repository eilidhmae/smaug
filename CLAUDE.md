...
**Phases 1–4 complete. Phase 5 Tiers 1–4 (foundation + flag-honoring + mudprog depth + content breadth) complete.** ~1,914 top-level tests — all passing across 13 packages. Boot loads 1,909 rooms, 4,299 exits, 505 mob templates, 821 obj templates, 406 mob instances, 710 obj instances, 1,603 helps, 325 skills/spells, 17 classes, 15 races, 8 clans, 2 deities, 496 socials, 2 boards.
...
- **Tier 4 — Content breadth.** ✓ Complete. `combat.DamMessage` ported from C new_dam_message with 18×24 verb tables. +24 custom-port spells (acid/fire/frost/gas/lightning breath, earthquake, teleport family, animate_dead, call_lightning, control_weather, etc.). +27 combat/utility skills (bite/claw/punch/sting/tail, circle/gouge/stun/grapple/cleave/hitall/berserk, meditate/search/detrap/dig/visible/style/stance/scribe/cook/feed/slookup/sset/mistwalk/skin/poison_weapon/fire). +16 immortal commands (switch/return, wizlock, shutdown/reboot, wizhelp, aecho, hell, log, mpstat/opstat/rpstat, deny, pardon, disconnect, mortalize). +8 mortal commands + alias/unalias subsystem with persistence (split, light, throw, areas, altscore, color, compress). OLC: +10 redit subcommands, flat oedit/medit (interactive substate deferred), rdelete/odelete/mdelete with repeat-confirm, mpedit/opedit/rpedit read-only inspectors. Five adversary rounds on G1/G5/G6/G4/G3 caught 14 load-bearing findings (skill hit fallthrough, DoHell state-before-guard, DoSwitch missing ACT_STATSHIELD, DoDeny no-kick, DoPardon missing attacker, DoLight wrong value slot, DoSplit missed coin type, alias exact-match vs prefix, DoCook wrong item type + no fire-in-room, DoPoisonWeapon wrong field, DoScribe wrong value check, DoHitall no group skip, breath spells wrongly AoE, acid missing container dissolve, animate_dead missing -5 sentinel) — all fixed pre-land with TDD. See `smaug-go/doc/phase5-tier4-content.md`.
