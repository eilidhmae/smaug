# SMAUG MUD

## Prime Directives (override all other rules)

1. **Assume the manager role defined in `@.claude/agents/manager.md` and follow its Prime Directives in full — they carry equal weight to the ones here.**
2. **On session start, read these foundation docs to resume context:**
   - `@smaug-go/doc/plan.md` — architecture, C→Go mapping, testing strategy
   - `@smaug-go/doc/phases.md` — phase roadmap and overall status
   - The most recent phase record (see the index under *Active Work* below)
3. **Consult other `smaug-go/doc/` files on demand via the index — do not preload the whole directory.**

---

SMAUG (Simulated Medieval Adventure Multi-User Game) — a text-only MMORPG on the Diku → Merc → SMAUG lineage. Active work is the pure-Go port in `smaug-go/`; original C in `src/` is reference only. Game data lives in `db/`. Current phase status is in `smaug-go/doc/phases.md`.

## Active Work: C → Go port

### Documentation index (`smaug-go/doc/`)

Read foundation docs every session; fetch phase records only when touching that phase; consult review docs on demand.

**Foundation:**

| File | Contents |
|---|---|
| `plan.md` | Architecture, C→Go mapping, design decisions, testing strategy |
| `phases.md` | All 5 phases: deliverables, verification criteria, current status |

**Phase records:**

| Phase | Plan / scope | Completion record |
|---|---|---|
| 1 | — | `phase1-completed.md`, `phase1-remaining.md` |
| 2 | `phase2-remaining.md` | `phase2-completed.md` |
| 3 | `phase3-plan.md` | `phase3-completed.md` |
| 4 | `phase4-plan.md` | `phase4a-completed.md`, `phase4b-completed.md` |
| 5 Tier 1 (Act/spell_smaug/saves/persistence) | `phase5-tier1-foundation.md` | `phase5-tier1-completed.md` |
| 5 Tier 2 (flag honoring + idle-data wiring) | `phase5-tier2-wiring.md` | `phase5-tier2-completed.md` |
| 5 Tier 3 (mudprog depth) | `phase5-tier3-mudprog.md` | `phase5-tier3-completed.md` |
| 5 Tier 4 (content breadth) | `phase5-tier4-content.md` (plan + completion) | — |
| 5 Tier 5 (test client + boot extraction) | `phase5-tier5-testclient.md` (plan + completion) | — |
| 5 Tier 6 (combat depth — multi-attack + prof bonus + stance) | `plan-combat-depth.md` (plan + completion) | — |
| 5 Tier 7 (dammessage gaps — was_in_room + PCFLAG_GAG + poison prefix) | `plan-dammessage-gaps.md` (plan + completion) | — |
| 5 Tier 8 (player-config commands — save / afk / title / password + pagelen) | `plan-player-config.md` (plan + completion) | — |
| 5 Tier 9 (communication channels — immtalk / gtell / auction stub) | `plan-channels.md` (plan + completion) | — |
| 5 Tier 10 (Deaf persistence fix + DoChannels toggle command) | `plan-do-channels.md` (plan + completion) | — |
| 5 Tier 11 (Timer subsystem + IsAttackSuppressed wiring + TIMER_RECENTFIGHT + DoQuit gate) | `plan-timer-subsystem.md` (plan + completion) | — |
| 5 Tier 12 (editor `/s` save — `EditorSave` callback + `CON_PLAYING` transition) | `plan-editor-save.md` (plan + completion) | — |
| 5 Tier 13 (`DoGag` standalone toggle) | `plan-do-gag.md` (plan + completion) | — |
| 5 Tier 14 (`rollD20` function-variable seam — eliminates combat test flake) | `plan-rollD20-seam.md` (plan + completion) | — |
| 5 Tier 15 (Tranche A quick wins — `DoBio`/`DoDescription`, `[AFK]` on who, `%X` prompt, retcode guards, testclient idiom polish, publicAll derivation) | — (ran without dedicated plan doc) | CHANGELOG.md 2026-04-18 entry |
| 5 Tier 16 (Tranche B — stances loader + DoMset stance branch + TIMER_DO_FUN callback registry + 7 mudprog if-checks + wordlist match + Oprog/Rprog CMD port) | `plan-tranche-b.md` (plan + completion) | — |
| 5 Tier 17 (Tranche C — quality/fidelity: `util.Act` per-call AType color + 33-site migration, `update_aris` audit correction, `PLR_BLANK` blank-line emission, XP-on-skill-gain + adept-cap message) | `plan-tranche-c.md` (plan + completion) | — |

**Phase 5 is complete (2026-04-18).** All 17 tiers + Tranches A/B/C landed; last commit `26db5f9`. **Phase 6 planning underway** — see `smaug-go/doc/phase6-roadmap.md` for the full candidate inventory, dependency graph, and recommended 6-wave execution order. The roadmap was externally audited 2026-04-18; 21 factual corrections applied (several "missing schema" claims were wrong — `MorphData`, `CharData.X/Y/Map/Sector`, `RoomIndexData.Plane`, `WEAR_MISSILE_WIELD`, `ITEM_PROJECTILE/QUIVER` are already defined).

**Phase 6 plans authored (pending external adversary review before execution):**

| Plan | Status | Scope |
|---|---|---|
| `plan-phase6-arena.md` | audited 2026-04-18 | Arena PvP — 7 groups, 15 criteria. Open Q6 (ROOM_ARENA area-data flag absent on vnums 10366-10382) is a confirmed prereq. |
| `plan-phase6-hotboot.md` | audited 2026-04-18 (PASS with notes) | Wave 0 design doc (not executable). Two Go designs evaluated with PoCs in `/tmp/` (both PASS). Recommends Design A (`syscall.Exec` + FD inheritance, ~65ms pause, seamless). 28 C citations and 19 Go-state claims verified; Windows infeasibility strengthened to double-lock (both `syscall.Exec` absent AND `(*net.TCPConn).File()` fd "not usable on other processes" per Go stdlib docs). `syscall.Dup2` (PoC) → `syscall.Dup3(old, new, 0)` needed for linux/arm64. Pre-G1 requirement: wire `HomeVnum` for `ACT_SENTINEL` mobs in `internal/handler/handler.go`. Executable rewrite follows after Open-Q1 (Windows support) resolves. |
| `plan-phase6-starmap.md` | **LANDED 2026-04-18** | `look sky` port — 3 groups, 13 criteria (A11 satisfied with documented fidelity deviation — eclipse render is a black-disk `" "` patch, not `&W@`, per C moonphase=0 masking). `internal/act/starmap.go` + `starmap_test.go` new (250 + 628 LOC); `internal/act/info.go` gains the `"sky"` branch after bare-arg room render and before keyword match. 8x72 `starMap` + 3x5 `sunMap` / `moonMap` verbatim from `src/starmap.c:59-85`. File-private constants (`starmapWeathUnit=10` etc.). Indoor check uses flag-OR-sector canonical pattern (`ROOM_INDOORS` OR `SECT_INSIDE`) matching `spell_unique.go:281-282` / `ifcheck.go:640-643`. 37 new tests green over `-count=3`, 7 `Edit`-round-trip mutations all caught (NUM_DAYS, weath_unit, byte-in-starMap, drop SECT_INSIDE clause, drop moonphase clamp, sun color, line-terminator). Commit `cd6f1b3`. |
| `plan-phase6-holidays.md` | audited 2026-04-18 (CONCERNS) | Holiday CRUD + persistence + `month_name[]` port — 4 groups, 13 criteria (+ A14/A15 pending Q6). Fixes 2 latent C bugs (day/month `<=1` rejection; 1-indexed file format). Audit corrected 2 material C claims: `get_holiday` has 2 live callers in `src/timezone.c` under `#ifdef ENABLE_HOLIDAYS` (plan claimed zero); no `do_load holiday` runtime-reload command exists (fabricated claim removed). New Q6: land `GetHoliday` + `DoTime` holiday-today suffix; season-tick announce deferred pending Go tick-hook grep. |
| `plan-phase6-marriage.md` | audited 2026-04-18 (CONCERNS) | marry/divorce/rings — 5 groups (G1-G5), 12 criteria. Cleans up `PCData.Spouse` orphan; fixes `SavePlayer` Spouse asymmetry. 5 open questions pending human input — critical: `newgate.are` vnum-100/101 collision invalidates original nil-guard strategy for `DoRings`; Q2 resolution broadened to 7 options. Re-audit after Q2 input. |
| `plan-phase6-planes.md` | **LANDED 2026-04-19** | Planes CRUD — 3 groups, 13 criteria. `LoadPlanes`/`SavePlanes`/`CheckPlanes` in new `internal/persist/planes.go` + `DoPlist`/`DoPstat`/`DoPset` + `planeLookup` in new `internal/act/planes.go`; boot wire registers three commands (Level 0 / LEVEL_IMMORTAL / LEVEL_GREATER) and calls `CheckPlanes(w, nil)` after loader. A1 empty-slice-vs-nil distinction implemented (missing file → `(nil, nil)`; stub `#END\n` → `(non-nil empty slice, nil)`). Tilde-tolerant Name reading accepts both C-format (space-terminated) and SMAUG-format (tilde-terminated); saver always emits tilde. Splice-delete algorithm (post-audit correction) handles first/middle/last index uniformly — `TestDoPset_DeleteMiddlePlane` + `TestDoPset_DeleteFirstPlane` + `TestDoPset_DeleteLastPlaneSymmetric` + `TestDoPset_DeleteLastPlaneRebuildsPrime` pin all four cases. SmashTilde on rename is deliberate C-divergence (C doesn't smash). 10 mutation gates verified via `Edit` round-trips (tilde-strip, name-empty rejection, Prime Material seed, deleted-pointer reassign, planeLookup pass-2, HasPrefix arg order, SmashTilde, CheckPlanes-after-splice, three registrations, boot CheckPlanes call). 36 new tests across `planes_test.go` files (persist: 19, act: 23, boot: 4, testclient E2E: 3). `go build ./...` clean, `go vet ./...` clean, `go test -count=3 ./...` green across all 15 packages. Commit `d895e5b`. |
| `plan-phase6-channels-extra.md` | **LANDED 2026-04-18** | `music`/`racetalk`/`wartalk`/`counciltalk`/`guildtalk`/`newbiechat` — shared `talkChannel` helper + 6 thin wrappers. 6 groups, 18 criteria, all satisfied. 7 mutation gates verified via `Edit` round-trips (wartalk/yell exception, self-echo-no-rebroadcast, filter application, pkill/council/guild/newbie gates, case-insensitivity). Helper in `internal/act/channels.go` owns PLR_SILENCE / deaf-sender (wartalk+yell exception with C's `||` bug corrected to `&&`) / ROOM_SILENCE / self-echo / broadcast; per-channel `recipientFilter` closures supply audience predicate. Tier 9 channels kept open-coded (non-retrofit). +~160 LOC impl, +~700 LOC tests. Stock `db/councils/council.lst` empty → `newbiechat` immortal-only by default matches C. Commit `5ff02b6`. |
| `plan-phase6-skills.md` | **LANDED 2026-04-18** | bloodlet/pounce/broach — 3 groups, 16/16 criteria satisfied. Ported `DoPounce` (`internal/act/skills3.go`), `DoBroach` (`internal/act/skills.go`), `DoBloodlet` + `isBloodRace` helper (`internal/act/skills4.go`), plus 3 `boot.go` registrations. 41 new tests all green (`-count=3`). Both C bugs fixed per Option-A (bloodlet operator-precedence: `!IS_NPC && (vampire || demon)`; broach tautology: `CLOSED && LOCKED && !PICKPROOF && can_use`). Bloodlet Wait uses literal `PULSE_VIOLENCE` not `Beats` per C :3535. Mutation-verify round-trips confirmed 5 mutations (weapon-type set, broach predicate, bloodthirst decrement, Wait constant, reverse-exit removal). Commit `1d929e8`. |
| `plan-phase6-stances-olc.md` | audited 2026-04-18 (CONCERNS) | Combat Stances OLC completion — 5 groups (G1-G5), 20 criteria, 10 open questions. Extends `StanceInfo` from 3 to 19 fields; upgrades loader to store non-combat keys; adds `SaveStances` writer; ports `can_use_stance` + `update_stances`; rewrites `DoStance` state machine (picks up 4 currently-unreachable stances: viper/crane/crab/normal); adds `DoSTstat`/`DoSTset` OLC. Preserves 4 C bugs with pin-tests (class/race setter no-ops, dual inverted, class/race `IS_SET(mask, index)`); fixes 1 cosmetic C bug (`do_ststat` newline-every-4). Depends on Tranche B G1 (shipped). **Audit caught 1 correctness-critical finding + 2 citation corrections**: (a) §D3 `UpdateStances` mixes `int` CharData fields with `uint32` RIS_* constants — cast required at fixture/test sites, compile-error risk at G2 without inline note; (b) §Q6 inline RisflagNames draft table stops at "plus7" but `constants.go:196-217` defines 22 bits terminating in `magic`/`paralysis` (bits 20-21); (c) Q9 `stance` registration verified present at `boot.go:521` (POS_DEAD / Level 0). **Q8 resolved**: `World.DataDir` confirmed at `world.go:76`. 4 in-plan surgical edits applied. |
| `plan-phase6-auction.md` | audited 2026-04-18 (CONCERNS) | Full `do_auction` state machine (non-GSC variant). 10 task groups, 23 criteria, 8 open questions. Completes Tier-9 `BroadcastAuction` stub by porting C `act_obj.c:3775-4240` + `update.c:3183-3326`. Adds `pulseAuction` slot; ports `parsebet`/`advatoi`/`num_punct`; extends `AuctionData` with history ring + timer. **Audit corrected 4 factual errors** in 4 in-place edits: (a) anomaly #1 carry-weight check is NOT dead code (seller can acquire weight during 27s window); (b) anomaly #2 C has active `DoQuit` gate at `act_comm.c:2883-2890` (not just `mud.h` comments) — gate wording now ported verbatim; (c) anomaly #14 `ms_find_obj` is a generalized drunk/mental-state check (`handler.c:2941`), not a mud-school quest hook; (d) §D7 C ships `do_noauction` admin command at `act_wiz.c:11120-11174` — load-only port honestly tracked as deferred follow-up. Pre-existing `pulseSave` uninitialized bug at `loop.go:68-94` flagged. |
| `plan-phase6-clan-officer.md` | audited 2026-04-18 (PASS after 3 edits) | Clan officer commands — induct / outcast / bestow + `SaveClan` bundled as G0. 5 task groups (+1 conditional), 14 criteria, 4 open questions. **Scope-correction verified independently**: `do_promote`/`do_demote` do NOT exist in shipped SMAUG (roadmap error at `phase6-roadmap.md:205`); rank management via `do_setclan`, captured as optional G5 (Q1 — human decision pending). Audit applied 3 surgical edits: added `Abbrev` to the loader-missing-keys list (originally omitted); flagged pre-existing legacy-PKills index bug (Go stores to `[0]`, C stores to `[6]` at `fread_clan:435`) for in-plan fix; neutralized one-sided Q1 bias (Option 1 G5-include or Option 2 defer-to-setclan both defensible). Actual C LOC ~478 (roadmap ~300). |
| `plan-phase6-olc-redit.md` | audited 2026-04-18 (PASS minor CONCERNS) | Interactive Room Editor (`CON_REDIT`) — 13 groups, 18 criteria, 6 open questions. First of three OLC-substate plans; proves nanny-dispatch pattern that `oedit`/`medit` will inherit. `OlcData` struct on `DescriptorData` (session-lifetime match); extends `DoRedit` to open menu on no-arg; flat subcommands preserved. Builds on Tier 12 `EditorSave`. Identifies `REDIT_EXIT_VNUM` asymmetry at `src/oredit.c:860-875` (C assigns `pexit->vnum` but not `pexit->to_room`) — plan fixes in Go. Audit applied 2 surgical edits: G5 EditorSave wording clarified (closure re-sets `CON_REDIT` AFTER StopEditingFunc since `/s` handler at `game/editor.go:163-172` transitions to CON_PLAYING before invoking closure); G12 testclient-prompt-tracking note added (menu has no `> ` sentinel, match on substring). |
| `plan-phase6-polymorph.md` | audited 2026-04-18 (PASS with CONCERNS, self-review) | Polymorph subsystem — 7 groups (G0-G6), 20 criteria, 7 open questions. **Schema complete, verified**: char_morph 32 fields + morph_data 65 fields spot-checked against Go `CharMorph`/`MorphData` (`character.go:270-372`). **Combat hook = direct stat mutation** in new `internal/handler/polymorph.go` (NOT AffectModify — scope limits prevent expressing immune/resistant/suscept masks or current-pool deltas). **Audit applied 5 corrections**: G4 morphset LOC 700→889 (G4a/G4b split mandatory); G6 Vnum-before-Name pfile ordering documented; G0.2 DiceParse exists (package-private `magic.parseDiceExpr` — plan now promotes not reimplements); G0.3 UpdateAris dropped (Go invariant: incremental `AffectModify`); G2.4 resource-leak ordering note in `do_morph_char`. `AT_MORPH` + `COND_BLOODTHIRST` already in Go (`enums.go:208,884`). Mudprog stubs at `commands.go:545-554`. |
| `plan-phase6-archery.md` | audited 2026-04-18 (CONCERNS) | DoDraw/DoFire/DoDislodge + arrow-lodge mechanic — 6 groups, 15 criteria. **3 C bugs preserved verbatim**: `do_draw` WEAR_DUAL_WIELD omission (`:142-149`); `do_dislodge` arm `3*v[1]..2*v[2]` asymmetry vs rib's 3/3 and leg's 2/2 (`:234`); `do_fire` never-assigned `victim==ch` check — structurally dead (`:1255,1272`). **OMIT combat-loop hook confirmed** (`grep WEAR_MISSILE_WIELD src/fight.c` zero hits; C archery is strictly `do_fire`-driven). **Audit schema-gap corrections**: `ROOM_NOMISSILE` already at `enums.go:722`, `PLR_NICE` at `:975`, `findDoor` at `move.go:12`, `CharData.NumFighting` at `character.go:78` (Q6/Q7/Q8 resolved). **New gap found**: `ITEM_WEAR_MAX=21` at `constants.go:588` must bump to 24 after WEAR_LODGE_* added. **Q12/Q13 added 2026-04-18 during readiness vet**: `mob_fire:1352` uses `bow->value[4]!=arrow->value[5]` vs `do_fire:1295` uses `[5]!=[4]` (value-index asymmetry; `mob_fire` is out of scope, `do_fire` ports `[5]!=[4]` verbatim — reconcile when `mob_fire` follow-up lands); lodged arrows can be bypassed via `remove` — C doesn't set ITEM_NOREMOVE (port verbatim; policy question for future builders). G7 quiver auto-reload cut. |
| `plan-phase6-housing.md` | authored 2026-04-18 (external adversary pending) | Player housing / apartments — 11 groups (G0-G10), 18 criteria, 10 open questions. 730 doc lines. Six commands: `do_house`/`do_gohome`/`do_residence`/`do_accessories`/`do_homebuy`/`do_sellhouse`. `HOME_DATA`/`HOMEBUY_DATA`/`ACCESSORIES_DATA` struct citations corrected to `src/house.h:66-118` (roadmap said `mud.h`). Uses Tier 12 `EditorSave` callback for per-room description customization. Soft-depends on hotboot (serialization complexity). **Q3 (persist.LoadPlayer offline support), Q6 (`homebuy_update` pulse cadence — grep `src/update.c` at G8 start), Q9 (`gohome`-from-arena policy)** are pre-G-start human gates. R7/Q2 deferred to executor grep (`SUB_ROOM_DESC` + `ITEM_HOUSEKEY`). |
| `plan-phase6-overland-loader.md` | authored 2026-04-18 (external adversary pending) | Overland loader + display — 13 groups (G1-G13), 17 criteria. ~820 doc lines. Raw-binary map file format (new loader shape; `persist.Scanner` is line-oriented so overland adds a sibling `persist.BinScanner`), sector-type lookup, coordinate utilities, `coords`/`survey`/`landmarks` player commands, `display_map` ANSI rendering. **Critical sector-enum migration**: Go `enums.go:549-567` has 16 `SECT_*` values in non-`OVERLANDCODE` order (AIR/UNDERWATER swapped vs overland's enum) — Q3 is 3-option decision (translation table / version field / bulk re-save) gating G1 start. Stub `MapData` at `room.go:105-114` (`Vnum int` only) replaced with full 4-field C `map_data`. Q1 (`mapreset` missing-`break` C bug preserve-vs-fix), Q2 (`ROOM_MAP` bit collision audit), Q4 (`PLR_ONMAP`/`ACT_ONMAP`/`PLR_MAPEDIT`/`ITEM_ONMAP`/`ACT_SENTINEL`/`ROOM_MAP` enum-presence audit) queue at G1. `MAP_INDEX_DATA` at `mud.h:6480` declared but unused in stock code path — out of scope with deferral note. |
| `plan-phase6-overland-builder.md` | authored 2026-04-18 (external adversary pending) | Overland admin editor — 12 groups (G0-G12), 15 criteria. ~720 doc lines. G0 is a pre-flight audit of loader completion; rest ports `setmark`/`setexit`/`mapresets`/`mreset`/`mapedit` admin commands + landing-site persistence. Blocked on overland-loader landing. **Q1 (reload_map + undo-stack race)** is pre-G-start human gate — recommended purge. |

**Remaining Phase-6 prerequisites:** none blocking Wave 1; Wave 2+ blockers covered by per-plan open questions. **17 Phase-6 plans authored as of 2026-04-18** — 14 audited (Wave A: arena, hotboot, starmap, holidays, marriage; Wave B: planes, channels-extra, skills; Wave C: stances-olc, auction, clan-officer; Wave D: olc-redit, polymorph, archery) + 3 pending external adversary (housing, overland-loader, overland-builder). **Still unauthored:** `plan-phase6-olc-oedit.md` + `-medit.md` (soft-blocked on olc-redit landing one executable task group), `plan-phase6-olc-mpedit.md` (hard-blocked on all three editors), `plan-phase6-dragonflight.md` (hard-blocked on overland-loader), `plan-phase6-deity-prayer.md` (Wave 6; roadmap claim of ~400 C LOC is wrong — `src/deity.c` is 1695 LOC, scope narrowing needed), `plan-phase6-foldarea.md` (deferred by policy). **Readiness-vet verdicts (2026-04-18)**: 12 READY to dispatch with first-30-minute notes, 1 BLOCKED (marriage — Q2 vnum-100/101 collision needs human input), 1 design-doc-only (hotboot — Q1 Windows support gate before executable rewrite). 3 surgical plan edits applied during vet: holidays A14/A15 added (Q6 resolution), planes A1 empty-slice vs nil clarified, archery Q12/Q13 added. Manager subagent harness `Agent`-tool availability remains inconsistent — all 2026-04-18 audits/authorings fell back to structured self-review; tooling investigation tracked in TODO.md. Small follow-up items in `TODO.md` Active — deferred mudprog ifchecks (`isflagged`/`istagged`), per-player color customization, `DoBlank` toggle, `DoConfig` surface, SmashTilde-ordering decision for planes rename, `combat.Damage` self-target safety verification before bloodlet G3, Wave D items (`ITEM_WEAR_MAX` bump for archery, lodged-arrow-remove policy, mob_fire value-index asymmetry, morphset G4a/G4b split).

**Reviews:**

| File | Contents |
|---|---|
| `security-review.md` | 16 findings, 14 fixed |
| `go-idiom-review.md` | Go idiom audit: 10 findings, all resolved |
| `audit-2026-04-17.md` | Post-Phase-5 4-adversary audit: phase completeness, idiom, security/usability, C↔Go capability — verdict CONCERNS |

## Build and run

```bash
cd smaug-go
go build -o smaug-go ./cmd/smaug/
./smaug-go -port 4000 -data ../db
# connect: telnet localhost 4000
```

Module: `github.com/eilidhmae/smaug`. Build all: `go build ./...`. Test all: `go test ./...`.

On fresh clone: `make install-hooks` (from `smaug-go/`) activates the pre-commit gate (gofmt + vet + unit tests + conflict markers). Bypass with `--no-verify`; skip individual gates with `SMAUG_SKIP_VET=1` / `SMAUG_SKIP_TESTS=1`.

## Conventions (not covered in `plan.md`)

- C enums → untyped `int` constants (not typed Go enums) so they interop cleanly with struct fields
- Struct fields use PascalCase; constant names match C exactly for cross-reference (`ACT_IS_NPC`, `PULSE_VIOLENCE`, `ROOM_VNUM_TEMPLE`)
- `types/` has no logic dependencies — everything else may import it
- `world.World` is passed explicitly, never a global. `act.WorldRef` is a package-level set-at-boot exception for commands.
- File loaders: `util.Bug()` and continue on bad data (don't abort — matches C behavior)
- No Cgo. Use pure Go / stdlib equivalents (e.g. `compress/zlib` for MCCP)

## TDD (mandatory)

`plan.md`'s Testing Strategy section points here for workflow details.

1. Write failing test → confirm red
2. Write minimum code to pass → confirm green
3. Refactor → confirm still green
4. Commit

For existing code, backfill tests and verify with **mutation testing**: stage working code (`git add`), mutate the implementation (flip a condition / change a return), run tests → must fail, revert (`git checkout`), run tests → must pass.

Tests live next to code (`foo.go` → `foo_test.go`). Fixtures go in `testdata/`. Aim for high coverage on new/modified code: `go test -cover ./...`.
