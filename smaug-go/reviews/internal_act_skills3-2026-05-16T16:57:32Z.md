...
**Timestamp**: 2026-05-16T16:57:32Z
...
### Complexity Audit
- The `unarmedAttack` function is reused across multiple skills (bite/claw/punch/sting/tail), which is good for maintainability.
- The `DoHitall` function has complex logic involving group membership checks and handling of retcodes from combat functions.
- Several functions have multiple conditions and branches that could be simplified or refactored for better readability.

### Scope Check
The changes appear to be focused on implementing various combat skills without introducing any additional features beyond what's described in the comments.

### Alternative Approach
For `DoHitall`, instead of manually copying the People slice and filtering out group members, a more idiomatic Go approach would be to use a filter function or filter the slice directly.

### Assumptions
- The `WorldRef` global variable exists and is properly initialized elsewhere in the codebase.
...
findings:
  - id: F1
    ...
    line_end: 35
    message: >
      The `unarmedAttack` function uses a hardcoded string for skill name,
      which could lead to typos or inconsistency if skill names change.
    suggested_fix: >
      Consider using constants for skill names to ensure consistency and
      prevent typos.
  - id: F2
    severity: minor
    category: maintainability
    ...
    line_end: 40
    message: >
      The comment mentions that "MVP: skip per-class level gating" but does not
      address whether this is intentional or an oversight.
    suggested_fix: >
      Clarify whether this behavior is intentional or needs to be addressed in
      a future update.
  - id: F3
    severity: minor
    category: maintainability
    ...
    line_end: 170
    message: >
      The comment mentions "Omissions vs. C" but doesn't provide specific details
      about what's missing from the implementation.
    suggested_fix: >
      Add more detailed comments explaining why certain features were omitted
      and how they might be implemented in the future.
findings:
  - id: F1
    severity: major
    category: maintainability
  ...
  - id: F3
    ...
    line_end: 40
    message: >
      The comment mentions "MVP: skip per-class level gating" but does not
      address whether this is intentional or an oversight.
    suggested_fix: >
      Clarify whether this behavior is intentional or needs to be addressed in
      a future update.
...
findings:
  ...
  - id: F2
    ...
    line_end: 170
    message: >
      The comment mentions "O omissions vs. C" but doesn't provide specific details
      about what's missing from the implementation.
    suggested_fix: >
      Add more detailed comments explaining why certain features were omitted
      and how they might be implemented in the future.
